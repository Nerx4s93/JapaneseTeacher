...
internal sealed class AnimatedPressButton : Control
{
    ...
    private void DrawBody(Graphics graphics)
    {
        ...
        using var path = RoundedRectanglePathCreator.GetRoundRectanglePath(rectangle, Radius, Radius);
        if (_active)
        {
            if (_mouseDown)
            {
            ...
            else
            {
                ...
                rectangle.Y = 0;
                using var topPath = RoundedRectanglePathCreator.GetRoundRectanglePath(rectangle, Radius, Radius);
